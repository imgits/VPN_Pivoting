	...
	int mtu)
{
	...
	for (i = 0;; i++)
	{
		...
		err = RegQueryValueEx(key2, "Name", 0, 0, (byte*)adaptername, &len);
...
/* ************************************************ */
void dumphex(char* note, char* databuf, int datasize)
{
	...
	for (int i = 0; i < datasize;i+=16)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			if (i + j < datasize && data[i + j] >=' ' && data[i + j] <= 0x7e)
			{
				putchar(data[i + j]);
			}
			else putchar('.');
